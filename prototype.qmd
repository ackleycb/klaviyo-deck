...
make_grid <- function(q_prefix, group_var, color = bar_color) { tryCatch({
  ...
  # Use make_1d_mat for all percentages (force_to_100 = TRUE)
  r_all <- make_1d_mat(dta = dat_edit, q_prefix = q_prefix, var_dict = var_dict, force_to_100 = TRUE)
  overall <- tibble(Option = as.character(r_all[[1]]), Overall = as.numeric(r_all$share_perc))
  by_g <- bind_rows(lapply(groups, function(g) {
    d_g <- dat_edit %>% filter(.data[[group_var]] == g)
    r_g <- make_1d_mat(dta = d_g, q_prefix = q_prefix, var_dict = var_dict, force_to_100 = TRUE)
    tibble(!!group_var := g, Option = as.character(r_g[[1]]), pct = as.numeric(r_g$share_perc))
