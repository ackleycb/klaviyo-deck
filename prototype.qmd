...
make_table <- function(q_prefix) {
  r <- make_1d_mat(dta = dat_edit, q_prefix = q_prefix, var_dict = var_dict, force_to_100 = TRUE)
  tbl <- tibble(Option = as.character(r[[1]]), `%` = as.numeric(r$share_perc)) %>% arrange(desc(`%`))
  tbl <- tbl %>% mutate(Option = stri_trans_general(Option, "Latin-ASCII"))
  disp <- tbl; disp$`%` <- paste0(disp$`%`, "%")
  k <- kbl(disp, format = "html", escape = TRUE, align = c("l","r")) %>%
  ...
  column_spec(k, 2, background = shades)
}
